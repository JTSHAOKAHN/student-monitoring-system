...
-- Create question_bank table BEFORE questions table to avoid foreign key issues
...
-- Add foreign key to question_bank for extracted_content if table exists
do $$
begin
  if exists (select 1 from information_schema.tables where table_name = 'extracted_content') then
    alter table public.question_bank 
    add constraint if not exists question_bank_source_content_id_fkey 
    foreign key (source_content_id) references public.extracted_content(id) on delete set null;
  end if;
  
  if exists (select 1 from information_schema.tables where table_name = 'pdf_uploads') then
    alter table public.question_bank 
    add constraint if not exists question_bank_source_document_id_fkey 
    foreign key (source_document_id) references public.pdf_uploads(id) on delete set null;
  end if;
end $$;
